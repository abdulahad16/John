		...
		if ( is_array($woo_options) ) {
			...
			if ( isset( $woo_options['woo_texttitle'] ) && $woo_options['woo_texttitle'] == 'true' ) {
				...
				if ( $woo_options['woo_tagline'] == "true" AND $woo_options['woo_font_tagline'] )
					$output .= '#header .site-description {'.woo_generate_font_css($woo_options[ 'woo_font_tagline'], '1', true ).'}' . "\n";
			}
...
if (!function_exists( 'woo_generate_font_css')) {
	function woo_generate_font_css($option, $em = '1', $important = false) {
		...
		$its_important = '';
		if ( $important ) {
			$its_important = ' !important';
		}

		if ( !@$option["style"] && !@$option["size"] && !@$option["unit"] && !@$option["color"] )
			return 'font-family: '.stripslashes($option["face"]). $its_important . ';';
		else
			return 'font:'.$option["style"].' '.$option["size"].$option["unit"].'/'.$em.'em '.stripslashes($option["face"]).';color:'.$option["color"]. $its_important .';';
	}
}
